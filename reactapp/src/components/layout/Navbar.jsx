                ...
                <div className="flex items-center gap-4 lg:gap-8">
                                                ...
                                                {link.subLinks.map((sub, subIndex) => (
                                                    ...
                                                    >
                                                        <span className="flex flex-col">
                                                            ...
                                                            {sub.desc && <span className="text-[10px] font-medium tracking-normal mt-0.5" style={{ opacity: 0.4 }}>{sub.desc}</span>}
                                                        </span>
                                                    </Link>
                                                ))}
                                            </div>
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div className="hidden sm:flex flex-col items-end gap-0.5 leading-none px-2">
                            <span className="text-[11px] font-bold tracking-tight" style={{ color: 'var(--theme-nav-text)' }}>{userName}</span>
                            <span className="text-[8px] font-black uppercase tracking-[0.1em]" style={{ color: 'var(--theme-accent)' }}>{role}</span>
                        </div>
                        ...
                        <div className="px-6 py-8 space-y-6">
                            {links.map((link, idx) => (
                                <div key={idx} className="space-y-4">
                                    <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'var(--theme-accent)' }}>{link.label}</p>
                                    {link.subLinks ? (
                                        <div className="grid grid-cols-1 gap-3 pl-2">
                                            {link.subLinks.map((sub, sIdx) => (
                                                ...
                                                >
                                                    <span className="font-bold text-sm" style={{ color: 'var(--theme-nav-text)' }}>{sub.label}</span>
                                                    {sub.desc && <span className="text-[10px]" style={{ color: 'var(--theme-text-muted)' }}>{sub.desc}</span>}
                                                </Link>
                                            ))}
                                        </div>
                                    ) : (
                                        <Link 
                                            to={link.path} 
                                            onClick={() => setIsMenuOpen(false)}
                                            className="block p-4 font-bold text-sm active:scale-95 transition-all"
                                            style={{ 
                                                background: 'var(--theme-accent-light)', 
                                                borderRadius: 'var(--theme-radius-lg)',
                                                border: '1px solid var(--theme-nav-border)',
